...
from auth import bp_auth, check_perm
...
def show():
    ...
    if current_user.is_admin():
        query = '''SELECT visit_logs.id, users3.first_name, users3.last_name, visit_logs.path, visit_logs.created_at, users3.login
...
@login_required
@check_perm('show_path_user')
def show_path_user():
    cursor = db.connection().cursor(named_tuple=True)
    #if current_user.is_admin():
    query = '''SELECT COUNT(*) as count, users3.first_name, users3.last_name, users3.login
        FROM visit_logs LEFT JOIN users3 on visit_logs.user_id = users3.id
        GROUP BY users3.first_name, users3.last_name, users3.login ORDER BY count DESC'''
    cursor.execute(query)
    #else:
...
def save_to_csv_func(FIELDS, logs):
    count = 0
    csv_data = ",".join(FIELDS) + "\n"
    for item in logs:
        count += 1
        csv_data += ",".join([str(count) if field == 'id' else str(getattr(item, field, '')) for field in FIELDS]) + "\n"
    file = BytesIO()
...
def save_to_csv_visits():
    ...
    return save_to_csv_func(FIELDS, logs)
