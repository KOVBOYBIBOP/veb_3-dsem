...
@comments_blueprint.route('/<int:book_id>', methods=['GET', 'POST'])
@login_required
def comment_post(book_id):
    comment = db.session.query(Feedback).filter(Feedback.book_id == book_id, Feedback.user_id == current_user.id).first()
